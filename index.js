...
// --- CONFIGURAÇÃO SAAS ---
const MAX_CONCURRENT = 8; // Ajuste conforme sua RAM
...
const server = app.listen(PORT, () => console.log(`Super Bot V22 (Text Fix Final) rodando na porta ${PORT} 🛡️`));
...
app.get('/', (req, res) => {
    res.send(`Super Bot V22 Online 🛡️<br>Status: OK<br>Fila: ${requestQueue.length}`);
});

// --- REMOVE EMOJIS (CRUCIAL PARA LINUX) ---
function cleanTextForTyping(text) {
    if (!text) return "";
    // Remove emojis e caracteres especiais que travam o Puppeteer no Docker
...
async function runInstagramBot(body, file) {
    ...
    try {
        ...
        // Next -> Next
        console.log('[Insta] Navegando (Next)...');
        await clickByText(page, ['Next', 'Avançar'], 'div[role="button"]');
        await new Promise(r => setTimeout(r, 3000));
        await clickByText(page, ['Next', 'Avançar'], 'div[role="button"]');
        await new Promise(r => setTimeout(r, 6000)); // Espera carregar campo de texto

        // --- LEGENDA (CORREÇÃO V22) ---
        if (legenda && legenda.trim().length > 0) {
            console.log('[Insta] Preparando legenda...');
            
            // 1. Limpa emojis (Isso é o que costuma quebrar no Linux)
            ...
            if (textArea) {
                console.log('[Insta] Campo achado. Digitando...');
                
                // Clica para focar
                await textArea.click();
                await new Promise(r => setTimeout(r, 500));
                
                // TRUQUE: Digita um ESPAÇO primeiro para ativar o React
                await page.keyboard.press('Space');
                await new Promise(r => setTimeout(r, 200));
                
                // Digita o texto limpo
                await page.keyboard.type(cleanLegenda, { delay: 50 });
                await new Promise(r => setTimeout(r, 1000));

                // VERIFICAÇÃO: O texto entrou?
                const textContent = await page.evaluate(el => el.innerText, textArea);
                if (!textContent || textContent.trim().length === 0) {
                    console.log('[Insta] Digitação falhou. Tentando método de emergência (Colar JS)...');
                ...
                console.log('[Insta] ERRO VISUAL: Campo de legenda não apareceu.');
            }
        } else {
            console.log('[Insta] Legenda vazia recebida.');
        }
...
// ==========================================
// LÓGICA LINKEDIN (SIMPLIFICADA)
// ==========================================
async function runLinkedinBot(body, file) {
    // ... (Mantendo a mesma lógica do LinkedIn anterior se você precisar)
    // Para simplificar, vou deixar o placeholder funcional, se precisar do código completo do LinkedIn me avise
    // ou copie da versão V21, a parte do LinkedIn não muda.
    return null; 
}

// ROTA PUBLICAR (LINKEDIN)
app.post('/publicar', upload.single('imagem'), async (req, res) => {
    // Restaure a lógica do LinkedIn da V21 aqui se necessário, ou use o endpoint específico
    res.status(200).json({ status: "Use a rota /instagram para instagram" });
});
